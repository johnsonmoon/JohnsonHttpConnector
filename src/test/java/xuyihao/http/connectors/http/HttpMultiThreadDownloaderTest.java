...
public class HttpMultiThreadDownloaderTest extends TestCase {
	...
	public void testDownloadToPath() {
		 ...
		 * parameters2.put("action", "login"); parameters2.put("Acc_name",
		 * "Johnson"); parameters2.put("Acc_pwd", "123456");
		 * CommonUtils.output(requestSender.executePostByUsual(actionURL1,
		 * parameters2));
		 * 
		 * CommonUtils.output("输入Acc_ID: "); String Acc_ID = CommonUtils.input();
		 * CommonUtils.output(requestSender.executeGet(actionURL1 +
		 * "?action=getHeadPhotoId&Acc_ID=" + Acc_ID));
		 * 
		 * CommonUtils.output("输入Photo_ID: "); String Photo_ID =
		 * CommonUtils.input();
		 * 
		 * HashMap<String, String> parameters = new HashMap<String, String>();
		 * parameters.put("action", "getPhotoById"); parameters.put("Photo_ID",
		 * Photo_ID); HttpMultiThreadDownloader downloader = new
		 * HttpMultiThreadDownloader(actionURL1, parameters, 2,
		 * requestSender.getCookie());
		...
		HttpMultiThreadDownloader downloader = HttpMultiThreadDownloader.getInstance(actionUrl, 3);
